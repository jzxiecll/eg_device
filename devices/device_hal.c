 ...
 *
 */
 uint32_t eg_uart_read( uint8_t *buf, uint32_t len)
{
#if EJ_UART_DMA
 ...
 *
 */
 bool eg_uart_write( uint8_t *buf, uint32_t len)
{	  	
#if EJ_UART_DMA
...
int eg_uart_open(EJ_UART_ID_Type port_id, uint32_t baud)
{
...
int eg_uart_close(EJ_UART_ID_Type uartX)
{
	EJ_port_uart_close();
	return 0;
}
