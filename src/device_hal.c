...
static int EJ_port_uart_dma_init( EG_UART_ID_Type port, uint32_t baud)
{
	/*Need to do */
	int ret=0;
...
static int EJ_port_uart_poll_init(EG_UART_ID_Type port_id, uint32_t baud)
{
	/*Need to do */
...
*
*/
static int EJ_port_uart_open(EG_UART_ID_Type port_id, uint32_t baud)
{
#if EJ_UART_DMA
...
static int eg_uart_open(EG_UART_ID_Type port_id, uint32_t baud)
{
...
static int eg_uart_close(EG_UART_ID_Type uartX)
{
	return EJ_port_uart_close();
}
 ...
 *
 */
static uint32_t eg_uart_read( uint8_t *buf, uint32_t len)
{
#if EJ_UART_DMA
 ...
 *
 */
static bool eg_uart_write(uint8_t *buf, uint32_t len)
{	  	
#if EJ_UART_DMA
 ...
 int eg_device_open()
 {
	return eg_uart_open(EG_UART1_ID, 9600);
 }
 int eg_device_close()
 {
	return eg_uart_close(EG_UART1_ID);
 }
 
 uint32_t eg_device_read(uint8_t *buf, uint32_t len)
 {
		return eg_uart_read(buf,len);
 }
